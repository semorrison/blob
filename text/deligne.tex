...
As usual, fix a system of field $\cF$ and let $\bc_*$ denote the blob complex construction based on $\cF$.
Let $\hom(\bc_*(M_i), \bc_*(N_i))$ denote the morphisms from $\bc_*(M_i)$ to $\bc_*(N_i)$,
as modules of the $A_\infty$ 1-category $\bc_*(E_i)$ (see \S\ref{ss:module-morphisms}).
...
If, in analogy to Hochschild cochains, we define elements of $\hom(\bc_*(M), \bc_*(N))$
